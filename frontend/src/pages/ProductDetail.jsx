...
function ProductDetail() {
  ...
  useEffect(() => {
    const getProductDetails = async () => {
      try {
        const res = await axios.get(`http://127.0.0.1:8000/api/store/products/${id}/`);
  ...
  if (error) {
    return <div className="py-8 bg-white md:py-16 dark:bg-gray-900 antialiased"><p className="text-red-500">{error}</p></div>;
  }
      ...
      <div className="max-w-screen-xl px-4 mx-auto 2xl:px-0">
        <div className="lg:grid lg:grid-cols-2 lg:gap-8 xl:gap-16">
          <div className="shrink-0 max-w-md lg:max-w-lg mx-auto">
            <img className="w-full dark:hidden" src={`http://localhost:8000/${productDetail.image_url}`} alt={productDetail.name} />
            <img className="w-full hidden dark:block" src="https://flowbite.s3.amazonaws.com/blocks/e-commerce/imac-front-dark.svg" alt={productDetail.name} />
          </div>
